...
func Setup(e *echo.Echo, db *pgxpool.Pool, cfg *config.Config) {
	...
	syncService.SetGoalsService(goalsService)
	...
	protected.GET("/overview", overviewH.Get)
	...
	reports.GET("/gamification", reportsH.GetGamificationReport)
	reports.GET("/salary-plan", reportsH.GetSalaryPlan)
	reports.GET("/installment-timeline", reportsH.GetInstallmentTimeline)
