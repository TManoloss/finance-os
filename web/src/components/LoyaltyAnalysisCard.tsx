...
export default function LoyaltyAnalysisCard({ data }: LoyaltyAnalysisCardProps) {
  if (!data || !data.loyalty_stats || !data.top_loyal) return null;
  ...
  const abandonmentCost = data.abandonment_cost || 0;
  const loyaltyStats = data.loyalty_stats || {};
  const topLoyal = data.top_loyal || [];
  const insightNarrative = data.insight_narrative || "";
      ...
      <div className="p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-6 border-2 border-black bg-elevated/20">
             ...
             <div className="text-3xl font-black text-danger">R$ {abandonmentCost.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</div>
          ...
          <div className="p-6 border-2 border-black bg-accent-teal/10 col-span-2">
             ...
             <div className="grid grid-cols-4 gap-4">
                {Object.entries(loyaltyStats).map(([key, val]) => (
                  <div key={key}>
            ...
            {topLoyal.map((m, i) => (
              <div key={i} className="flex items-center justify-between p-4 border-2 border-black bg-elevated/10 group hover:bg-background transition-colors">
                ...
                <div className="text-right">
                   <div className="text-sm font-black">R$ {(m.clv || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</div>
                   <div className="text-[10px] font-bold text-text-secondary uppercase">{m.total_freq || 0}X VISITAS</div>
                </div>
              </div>
            ))}
          </div>
