...
async function getSummary(token: string, from?: string, to?: string) {
  ...
  try {
    let url = "/api/v1/transactions/summary";
    if (from && to) {
...
async function getReports(token: string) {
  if (!token) return [];
  try {
    const resp = await apiServer.get("/api/v1/reports", {
      headers: { Authorization: `Bearer ${token}` }
    });
    return resp.data?.data || [];
  } catch (error) {
...
async function getCashflow(token: string, from: string, to: string) {
  if (!token) return [];
  try {
    const resp = await apiServer.get(`/api/v1/reports/cashflow?from=${from}&to=${to}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
async function getFeed(token: string) {
  if (!token) return [];
  try {
    const resp = await apiServer.get("/api/v1/feed?page_size=5", {
      headers: { Authorization: `Bearer ${token}` }
    });
    return resp.data?.data || [];
  } catch (error) {
...
async function getLatestTransactions(token: string) {
  if (!token) return [];
  try {
    const resp = await apiServer.get("/api/v1/transactions?page=1&page_size=10", {
      headers: { Authorization: `Bearer ${token}` }
    });
...
async function getProjections(token: string) {
  if (!token) return null;
  try {
    const resp = await apiServer.get("/api/v1/reports/projection", {
      headers: { Authorization: `Bearer ${token}` }
    });
    return resp.data?.data;
  } catch (error) {
...
async function getUpcomingExpenses(token: string) {
  if (!token) return [];
  try {
    const resp = await apiServer.get("/api/v1/reports/upcoming-expenses", {
      headers: { Authorization: `Bearer ${token}` }
    });
    return resp.data?.data || [];
  } catch (error) {
...
async function getPersonalInflation(token: string) {
  if (!token) return null;
  try {
    const resp = await apiServer.get("/api/v1/reports/personal-inflation", {
      headers: { Authorization: `Bearer ${token}` }
    });
    return resp.data?.data;
  } catch (error) {
...
async function getSilentGrowth(token: string) {
  if (!token) return null;
  try {
    const resp = await apiServer.get("/api/v1/reports/silent-growth", {
      headers: { Authorization: `Bearer ${token}` }
    });
    return resp.data?.data;
  } catch (error) {
...
async function getMealCost(token: string) {
  if (!token) return null;
  try {
    const resp = await apiServer.get("/api/v1/reports/meal-cost", {
      headers: { Authorization: `Bearer ${token}` }
    });
    return resp.data?.data;
  } catch (error) {
...
async function getConvenience(token: string) {
  if (!token) return null;
  try {
    const resp = await apiServer.get("/api/v1/reports/convenience-index", {
      headers: { Authorization: `Bearer ${token}` }
    });
    return resp.data?.data;
  } catch (error) {
